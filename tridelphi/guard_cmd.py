...
# The optional tooling and what each one buys.
...
def _offer_missing_tools(
    _root: Path, *, yes: bool, stream: TextIO, out: TextIO
) -> None:
    """Name missing coverage, but never run installer code from the target.

    The repository being guarded is untrusted input. A file it calls
    ``scripts/install-ladder.sh`` is not a TriDelPhi installer, even if the name
    matches. Installation therefore stays a separate, user-run operation from
    an official TriDelPhi checkout or release.
    """
    ...
    print(
        ...
        "  For safety, guard never runs installer scripts from the project it is\n"
        "  scanning. Use the pinned, checksum-verified installer from the official\n"
        "  TriDelPhi release or checkout, then run guard again.\n",
        file=out,
    )
    if yes:
        print("  (`-y` only authorizes verified code fixes; it never installs tools.)\n", file=out)
